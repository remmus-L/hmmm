<template>
  <div>
    ...
    <!-- 新增目录弹窗 -->
    <el-dialog title="新增标签" :visible.sync="dialogVisible" width="26%">
      <el-form
        :model="ruleForm"
        :rules="rules"
        ref="ruleForm"
...
import { getDirectorysData, submitDialogData, getOptionData, changeBtnState, editDialogData, deleteCurrentRowData } from '@/api/cousemanger/tags'
export default {
  created () {
    // 获取表格数据
  ...
  methods: {
    ...
    async submitDialogData () {
      ...
      obj.tagName = this.ruleForm.tagName
      // 提交数据
    ...
    // 点击修改按钮触发
    async editProject (subjectID, id, tagName) {
      // 下拉框选中的值
      const res = await getOptionData()
      // 获取弹窗下拉框的数据
      this.ruleForm.options = res.data
      ...
      // 获取目录名称
      this.editRuleForm.tagName = tagName
